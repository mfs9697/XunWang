...
\begin{document}
	...
	\subsection{Design Process from Cubic Configuration to Curved Configuration of Truss Lattice Structures}
	
	To realize the precise transformation of traditional truss lattice configurations from regular cubic forms to cylindrical curved surfaces, a low-density cylindrical truss lattice structure is proposed. Figure~\ref{fig1} illustrates the design process used to transform truss lattice structures from cubic configurations to cylindrical curved-surface configurations. Through geometric adaptation and structural reconstruction, the transition to cylindrical curved-surface components is achieved to meet the requirements for impact resistance and energy absorption of curved components in aerospace and related fields.
	
	First, typical cubic truss lattice unit cells with distinct geometric symmetry and periodic arrangement are considered. Then, a series of geometric transformation and mapping operations are applied to gradually unfold the cubic unit cells and attach them to the cylindrical curved surface. During this process, the connectivity of the struts and the basic topological characteristics of the node distribution are retained. Meanwhile, the strut lengths and node positions are adjusted to form the cylindrical truss lattice structure. By properly controlling the strut diameter, node spacing, and interlayer arrangement, the resulting cylindrical lattice structure maintains excellent deformation resistance even at low density. This systematic design method, which transforms cubic configurations into cylindrical curved surfaces, provides a framework for subsequent mechanical modeling, numerical simulation, and experimental verification of cylindrical truss lattice structures.
	...
	In nature, plants such as wheat, dandelion, and horsetail can sustain their own weight owing to the excellent load-bearing capacity and deformation resistance of their stems. As shown in Figure~\ref{fig2}, staggered internal struts form periodic supporting frames. A cylindrical truss lattice structure can therefore be adopted to achieve uniform load transfer and enhance the overall deformation resistance. Accordingly, inspired by the superior stiffness characteristics of plant stems, this study designs several bio-inspired composite cylindrical truss lattice structures with excellent load-bearing performance and stiffness. Figure~\ref{fig2} also presents optical micrographs showing the cylindrical lattice morphology of plant stems. At low relative density, these cylindrical truss lattice structures fully exploit the advantages of low weight and high stiffness, and thus exhibit outstanding mechanical properties. This study provides useful design ideas for lightweight curved-surface structures in relevant engineering fields.
	...
	The design process and basic dimensional parameters of the composite cylindrical truss lattice structures are illustrated in Figure~\ref{fig3}. The proposed structures are classified as body-centered cubic cylindrical lattice structure, face-centered cubic cylindrical lattice structure, octahedral cylindrical lattice structure, and tesseract cylindrical lattice structure.
	
	\subsection{Relative Density of Cylindrical Truss Lattice Structures}
	
	The basic geometric parameters of the composite cylindrical truss lattice structures include the unit-cell height and the number of structural layers, denoted by $l_i$ and $n$, respectively, with $n=6$. They also include the outer and inner radii, denoted by $R_i$ and $r_i$, respectively. The struts of the cylindrical lattice structures have circular cross-sections, and the diameter of each strut is denoted by $d_i$. Here, $i=1,2,3,4$ correspond to the body-centered cubic, face-centered cubic, octahedral, and tesseract cylindrical lattice structures, respectively. The relationships between the main geometric parameters are as follows:
	%
	\begin{equation}
	...
	For the body-centered cubic cylindrical lattice structure, the relative density $\bar{\rho}_1$ can be expressed as
	%
	\begin{equation}
		\bar{\rho}_1 =
		\frac{
	...
	\end{equation}
	%
	In Eq.~\eqref{Eq4}, $N_{1,1}$ and $N_{1,2}$ denote the numbers of inclined struts, $l_{1,1}$ and $l_{1,2}$ denote the corresponding strut lengths, and $V_{c1}$ is the overlapping volume of the body-centered cubic cylindrical lattice structure. These quantities are given by
	%
	\[
	\begin{aligned}
		N_{1,1} &= 4n, &
	...
	For the face-centered cubic cylindrical lattice structure, the relative density $\bar{\rho}_2$ can be expressed as
	%
	\begin{equation}
		\bar{\rho}_2 =
		\frac{
	...
	\end{equation}
	%
	In Eq.~\eqref{Eq5}, $N_{2,j}$ and $l_{2,j}$ $(j=1,2,3,4)$ denote the numbers and lengths of the corresponding struts, respectively, and $V_{c2}$ is the overlapping volume of the face-centered cubic cylindrical lattice structure. These quantities are given by
	%
	\[
	\begin{aligned}
	...
	\begin{equation}
		\bar{\rho}_3 =
		\frac{
			12\left(A_3\sum_{j=1}^{9}N_{3,j}l_{3,j}\right)-V_{c3}
		}{
			nA_{30}l_3
		}.
		\label{Eq6}
	\end{equation}
	%
	In Eq.~\eqref{Eq6}, $N_{3,j}$ and $l_{3,j}$ $(j=1,\ldots,9)$ denote the numbers and lengths of the corresponding struts, respectively, and $V_{c3}$ is the overlapping volume of the octahedral cylindrical lattice structure. These quantities are given by
	%
	\[
	\begin{aligned}
		...
		% in the summation, but does not provide explicit definitions for
		% N_{3,7}, l_{3,7}, N_{3,8}, and l_{3,8}. These expressions should be
		% checked against the original derivation or source file.
		%
		N_{3,9} &= 4n, &
		l_{3,9} &=
		\frac{1}{2}
	...
	\end{equation}
	%
	In Eq.~\eqref{Eq7}, $N_{4,j}$ and $l_{4,j}$ $(j=1,\ldots,6)$ denote the numbers and lengths of the corresponding struts, respectively, and $V_{c4}$ is the overlapping volume of the tesseract cylindrical lattice structure. These quantities are given by
	%
	\[
	\begin{aligned}
	...
	Figure~\ref{fig4} presents the relationship between relative density and the geometric dimensions of composite cylindrical truss lattice structures. The relative density is governed by several geometric parameters, including the unit-cell height and strut diameter. For the four types of cylindrical truss lattice structures considered here, namely the body-centered cubic, face-centered cubic, octahedral, and tesseract structures, the relative density varies with the geometric parameters.
	
	As the outer radius increases within the considered range, the relative density generally shows an increasing trend. This indicates that a larger outer radius corresponds to a denser material distribution in the structure. The relative-density variation also differs among the four topologies. For example, the tesseract cylindrical lattice structure has the lowest relative density among the considered structures. This demonstrates that the structural topology has a significant influence on relative density. Owing to their distinct geometric configurations, the four structures exhibit different relative-density evolution trends when the same geometric parameters are adjusted. Therefore, the relative density can be regulated by adjusting geometric parameters, thereby providing a basis for further optimization of the mechanical properties.
	...
	The axial strain energies $U_b^p$ and $U_c^p$ along the inclined struts are expressed as
	%
	\begin{equation}
		\left\{
		\begin{aligned}
			U_b^p &= \frac{\left(F_b^p\right)^2\kappa_{1,1}}{2A_{10}E_s}, \\
			U_c^p &= \frac{\left(F_c^p\right)^2\kappa_{1,2}}{2A_{10}E_s}.
		\end{aligned}
		\right.
		\label{Eq13}
	\end{equation}
	
	The bending strain energies $U_b^v$ and $U_c^v$ perpendicular to the inclined struts are expressed as
	%
	\begin{equation}
		\left\{
		\begin{aligned}
			U_b^v &= \frac{\left(F_b^v\right)^2\kappa_{1,1}^3}{6E_sI_1}, \\
			U_c^v &= \frac{\left(F_c^v\right)^2\kappa_{1,2}^3}{6E_sI_1}.
		\end{aligned}
		\right.
		\label{Eq14}
	\end{equation}
	
	The total strain energy $U_1$ of the body-centered cubic cylindrical lattice structure is expressed as
	%
	\begin{equation}
		U_1 = 24U_a + 48\left(U_b^p+U_b^v+U_c^p+U_c^v\right).
		\label{Eq15}
	\end{equation}
	
	The equivalent strain energy $U_{\mathrm{eq}1}$ of the body-centered cubic cylindrical lattice structure is expressed as
	%
	\begin{equation}
		U_{\mathrm{eq}1}
	...
	Based on the energy method, the total strain energy $U_1$ of the body-centered cubic cylindrical lattice structure is equal to its equivalent strain energy $U_{\mathrm{eq}1}$, namely
	%
	\begin{equation}
		U_1 = U_{\mathrm{eq}1}.
		\label{Eq17}
	...
	\begin{equation}
		\begin{aligned}
			...
			{16\left(R_1^2-r_1^2\right)\kappa_{1,1}^5}
			+\frac{9d_1^4l_{1,2}^2l_1}
			{16\left(R_1^2-r_1^2\right)\kappa_{1,2}^5} \\
			&+\frac{27d_1^6l_1^3}
			{1024\left(R_1^2-r_1^2\right)\kappa_{1,1}^7}
			+\frac{27d_1^6l_1^3}
			{1024\left(R_1^2-r_1^2\right)\kappa_{1,2}^7}
			\Bigg]E_s .
		\end{aligned}
		\label{Eq18}
	\end{equation}
	
	As shown in Figure~\ref{fig5}(b), following the same procedure as that used for the body-centered cubic cylindrical lattice structure, the compressive modulus $E_2^*$ of the face-centered cubic cylindrical lattice structure is obtained as
	%
	\begin{equation}
		\begin{aligned}
			E_2^*
			...
			+\frac{9d_2^4\left(R_2-r_2\right)^2l_2}
			{2l_{2,4}^5\left(R_2^2-r_2^2\right)}
			+\frac{27d_2^6l_2^3}
			{8l_{2,4}^7\left(R_2^2-r_2^2\right)} \\
			&+\frac{18d_2^4\kappa_{2,1}^2l_2}
			{l_{2,1}^5\left(R_2^2-r_2^2\right)}
			+\frac{18d_2^4\kappa_{2,2}^2l_2}
			{l_{2,2}^5\left(R_2^2-r_2^2\right)} \\
			&+\frac{6d_2^2l_2^3}
			{l_{2,1}^3\left(R_2^2-r_2^2\right)}
			+\frac{8d_2^2l_2^3}
			{l_{2,2}^3\left(R_2^2-r_2^2\right)}
			\Bigg]E_s .
		\end{aligned}
		\label{Eq19}
	\end{equation}
	...
	As shown in Figure~\ref{fig5}(c), the corresponding compressive modulus $E_3^*$ of the octahedral cylindrical lattice structure is expressed as
	%
	\begin{equation}
		\begin{aligned}
			E_3^*
			= \Bigg[
			&\frac{9d_3^4\left(R_3-r_3\right)^2l_3}
			{64l_{3,4}^5\left(R_3^2-r_3^2\right)}
			+\frac{27d_3^6l_3^3}
			{1024l_{3,4}^7\left(R_3^2-r_3^2\right)} \\
			&+\frac{9d_3^4\kappa_{4,1}^2l_3}
			{16l_{3,1}^5\left(R_3^2-r_3^2\right)}
			+\frac{9d_3^4\kappa_{4,2}^2l_3}
			{16l_{3,2}^5\left(R_3^2-r_3^2\right)} \\
			&+\frac{3d_3^2l_3^3}
			{4l_{3,1}^3\left(R_3^2-r_3^2\right)}
			+\frac{3d_3^2l_3^3}
			{4l_{3,2}^3\left(R_3^2-r_3^2\right)} \\
			&+\frac{3d_3^2l_3^3}
			{8\kappa_{5,1}^3\left(R_3^2-r_3^2\right)}
			+\frac{3d_3^2l_3^3}
			{8\kappa_{5,2}^3\left(R_3^2-r_3^2\right)} \\
			&+\frac{9d_3^4\kappa_{6,1}^2l_3}
			{32\kappa_{5,1}^5\left(R_3^2-r_3^2\right)}
			+\frac{9d_3^4\kappa_{6,2}^2l_3}
			{32\kappa_{5,2}^5\left(R_3^2-r_3^2\right)} \\
			&+\frac{9d_3^4\kappa_{3,2}^2l_3}
			{16\kappa_{3,1}^5\left(R_3^2-r_3^2\right)}
			+\frac{27d_3^6l_3^3}
			{1024\kappa_{3,1}^7\left(R_3^2-r_3^2\right)}
			\Bigg]E_s .
		\end{aligned}
		\label{Eq20}
	\end{equation}
	...
	\begin{aligned}
		...
		\kappa_{6,1}
		&=
		\frac{\left(\sqrt{6}+\sqrt{2}\right)\left(R_3-r_3\right)}
		{4\left(R_3+r_3\right)}r_3, \\
		\kappa_{6,2}
		&=
		\frac{\left(\sqrt{6}+\sqrt{2}\right)\left(R_3-r_3\right)}
		{4\left(R_3+r_3\right)}R_3 .
	...
	\begin{equation}
		\begin{aligned}
			...
			{256l_{4,1}^6\left(R_4^2-r_4^2\right)}
			+\frac{27d_4^4\kappa_{7,2}^2l_4^2}
			{256l_{4,2}^6\left(R_4^2-r_4^2\right)} \\
			&+\frac{9d_4^4\kappa_{7,1}^2l_4}
			{64l_{4,1}^5\left(R_4^2-r_4^2\right)}
			+\frac{9d_4^4\kappa_{7,2}^2l_4}
			{64l_{4,2}^5\left(R_4^2-r_4^2\right)} \\
			&+\frac{27d_4^6l_4^3}
			{16384l_{4,1}^7\left(R_4^2-r_4^2\right)}
			+\frac{27d_4^6l_4^3}
			{16384l_{4,2}^7\left(R_4^2-r_4^2\right)}
			\Bigg]E_s .
		\end{aligned}
		\label{Eq21}
	\end{equation}
	...
	From Eq.~\eqref{Eq18}, the stress $\sigma_1$ of the body-centered cubic cylindrical lattice structure is expressed as
	%
	\begin{equation}
		\begin{aligned}
			\sigma_1
	...
	As shown in Figure~\ref{fig5}(a), the stress $\sigma_a$ of the axial strut is expressed as
	%
	\begin{equation}
	...
	The stresses $\sigma_b$ and $\sigma_c$ of the inclined struts are given by
	%
	\begin{equation}
		\left\{
		\begin{aligned}
	...
	Substituting Eqs.~\eqref{Eq23} and \eqref{Eq24} into Eq.~\eqref{Eq22}, the stress $\sigma_1$ of the body-centered cubic cylindrical lattice structure is derived as
	%
	\begin{equation}
		\begin{aligned}
			\sigma_1
			...
			\frac{
				...
				16\kappa_{1,1}^3
				\left(d_1l_1+16\kappa_{1,1}l_{1,1}\right)
			}
			\sigma_b \\
			&+
			\frac{
				...
				16\kappa_{1,2}^3
				\left(d_1l_1+16\kappa_{1,2}l_{1,2}\right)
			}
			\sigma_c
			\Bigg].
		\end{aligned}
	...
	Similar to the body-centered cubic cylindrical lattice structure, as shown in Figure~\ref{fig5}(b), the failure strength $\sigma_2^*$ of the face-centered cubic cylindrical lattice structure can be derived using the mixed failure criterion for the four types of struts as
	%
	\begin{equation}
		\begin{aligned}
			\sigma_2^*
			...
			+
			\frac{
				16d_2l_{2,4}^2\left(R_2-r_2\right)^2
				+3d_2^3l_2^2
			}{
				2l_{2,4}^3
				\left[
				d_2l_2+8l_{2,4}\left(R_2-r_2\right)
				\right]
			}
			\sigma_{b2}^{\mathrm{pk}} \\
			&+
			\frac{
				...
				l_{2,1}^2
				\left(
				3\kappa_{2,1}d_2+\frac{1}{2}l_2l_{2,1}
				\right)
			}
			\sigma_{c2}^{\mathrm{pk}} \\
			&+
			\frac{
				...
				l_{2,2}^2
				\left(
				3\kappa_{2,2}d_2+\frac{1}{2}l_2l_{2,2}
				\right)
			}
			\sigma_{d2}^{\mathrm{pk}}
			\Bigg\}.
	...
	\end{equation}
	%
	where the plastic buckling stresses $\sigma_{a2}^{\mathrm{pk}}$, $\sigma_{b2}^{\mathrm{pk}}$, $\sigma_{c2}^{\mathrm{pk}}$, and $\sigma_{d2}^{\mathrm{pk}}$ of the four types of struts are expressed as
	%
	\[
	\begin{aligned}
		\sigma_{a2}^{\mathrm{pk}}
		...
		\sigma_{b2}^{\mathrm{pk}}
		&=
		\frac{25\pi^2d_2^2}{49l_{2,4}^2}E_s, \\
		\sigma_{c2}^{\mathrm{pk}}
		&=
		\frac{\pi^2d_2^2}{4l_{2,1}^2}E_s, &
		\sigma_{d2}^{\mathrm{pk}}
		&=
		\frac{\pi^2d_2^2}{4l_{2,2}^2}E_s .
	\end{aligned}
	\]
	...
	\begin{equation}
		\begin{aligned}
			\sigma_3^*
			=
			\frac{3d_3^2}{4\left(R_3^2-r_3^2\right)}
			\Bigg[
			&\frac{
				64d_3l_{3,4}^2\left(R_3-r_3\right)^2
				+3d_3^3l_3^2
			}{
				4l_{3,4}^3
				\left[
				d_3l_3+16l_{3,4}\left(R_3-r_3\right)
				\right]
			}
			\sigma_{a3}^{\mathrm{pk}} \\
			&+
	...
	To further explore the mechanical properties of composite cylindrical truss lattice structures, verify the theoretical models for stiffness and strength proposed in the previous sections, and compare the predictions with experimental results, finite element analysis was adopted to conduct numerical simulations of several structural configurations. First, geometric models of the cylindrical truss lattice structures were established strictly according to the dimensions of the actual specimens, with the detailed dimensional parameters listed in Table~\ref{tab1}. Subsequently, mesh generation was performed for the finite element models, and the tetrahedral solid element C3D10M was used throughout this study.
	...
	Finite element models of four types of low-density cylindrical truss lattice structures were established using the Abaqus/Explicit analysis module to determine their elastic modulus and strength at different relative densities. Figure~\ref{fig6} presents the mesh convergence analysis results for the compressive strength obtained from finite element simulations. To improve the efficiency of the mesh convergence analysis for composite cylindrical truss lattice structures, the body-centered cubic cylindrical lattice structure was selected as the representative model, with geometric dimensions of $R_1=\SI{38}{mm}$, $r_1=\SI{22}{mm}$, $l_1=\SI{15}{mm}$, and $d_1=\SI{2}{mm}$. The simulation results show that variations in mesh size exert only a slight influence on the final analysis results. Accordingly, a mesh size of \SI{1}{mm} was adopted, which satisfies the accuracy requirements of the finite element simulation while maintaining overall computational efficiency.
	...
	An elastoplastic model was adopted to characterize the stiffness and strength of the cylindrical truss lattice structures under quasi-static compression, and the Dynamic, Explicit analysis step was employed in the finite element simulation. For the boundary conditions, the lattice specimens were placed between two rigid compression platens, which were defined as discrete rigid bodies in the model. The corresponding degrees of freedom and boundary constraints were assigned to their reference points (RPs). To reproduce uniform compression loading, the lower platen was fully fixed, whereas the upper platen moved downward along the $y$-direction, as illustrated in Figure~\ref{fig7}. General contact was defined between the platens and the lattice structures. Hard normal contact was specified, and the penalty method was used for tangential behavior with a friction coefficient of 0.15. The simulated elastic modulus of the composite cylindrical truss lattice structures was calculated from the slope of the stress--strain curve in the elastic deformation stage.
	
	To investigate the impact resistance of composite cylindrical truss lattice structures, finite element models were established to analyze their deformation and energy absorption characteristics under impact loading. The Abaqus/Explicit dynamic module was used for numerical modeling to compare the impact performance of different composite cylindrical truss lattice structures. The Johnson--Cook (J--C) plasticity model was adopted in the finite element analysis to describe the strain-rate hardening effect of the material under impact loading. Since thermal effects were not considered in the J--C constitutive model for the present dynamic impact simulation, the yield stress of the material under dynamic impact can be simplified as
	%
	\begin{equation}
		\sigma_c =
	...
	The contact properties and mesh generation for the impact simulations were identical to those used in the quasi-static compression simulations. For the interaction settings under impact loading, a mass of \SI{7.73}{kg} was assigned to the reference point of the upper steel plate, and a downward velocity was applied to impose impact loading on the cylindrical truss lattice structures and extract stress and strain data during the finite element simulation. The lower steel plate was constrained in the same manner as in the quasi-static compression tests.

\section{Experimental Methods for Composite Cylindrical Truss Lattice Structures}

Dog-bone specimens and composite cylindrical truss lattice structures with various configurations were fabricated by 3D printing using PA6 nylon reinforced with short glass fibers. Tensile tests were conducted on standard dog-bone specimens to establish the constitutive relation of the base material, which was then used in the numerical simulations. Quasi-static compression and low-velocity impact tests were performed to validate the simulation results and obtain experimental data for different composite cylindrical truss lattice structures.

\subsection{Fabrication of Composite Cylindrical Truss Lattice Structures}

The composite cylindrical truss lattice structures were fabricated from short-glass-fiber-reinforced PA6 nylon by selective laser sintering, with a laser power of \SI{200}{W}. After manufacturing, all specimens were sandblasted for \SI{20}{min} to remove residual PA6 nylon powder. The composite cylindrical truss lattice structures with different configurations are shown in Figure~\ref{fig8}. Four structural configurations were investigated in this study, and each structure consisted of six layers. The geometric dimensions corresponding to different relative densities are listed in Table~\ref{tab1}. The maximum dimensional error between the designed values and the measured specimens was less than 0.4\%.
...
Figure~\ref{fig9} shows the microstructure of the short-glass-fiber-reinforced PA6 nylon composite. The glass fibers exhibit slender filament-like shapes with a uniform diameter distribution and are well dispersed within the PA6 nylon matrix. Tight interfacial bonding between the glass fibers and the PA6 nylon matrix is also observed, with no obvious gaps or debonding. This indicates that load can be effectively transferred between the two phases.

\subsection{Characterization of the Base Material}

To characterize the mechanical properties of the short-glass-fiber-reinforced PA6 nylon, standard specimens were fabricated using the same printing method as that adopted for the lattice specimens, with a printing direction of $90^\circ$. The tensile and compressive properties of the base material were tested in accordance with ASTM D638-14 and ASTM D695-15, respectively. The density and Poisson's ratio of the composite were \SI{1.15}{g/cm^3} and 0.3, respectively. Tensile and compressive tests were carried out on the printed standard specimens using an INSTRON 5569 universal testing machine with a maximum load capacity of \SI{50}{kN} at a nominal strain rate of \SI{1e-3}{s^{-1}}. The true stress--strain curves of the tensile and compressive specimens are plotted in Figure~\ref{fig10}, and the relevant parameters are summarized in Table~\ref{tab2}.
...
To investigate the mechanical behavior of composite cylindrical truss lattice structures during the quasi-static elastic deformation stage, quasi-static compression tests were performed on four groups of lattice structures with different configurations using an Instron 5569 universal testing machine. The apparatus was equipped with an electronic displacement controller, a load sensor, a camera, a loading platform, and two rigid compression platens. During the tests, each lattice specimen was placed between the upper and lower rigid platens. The upper platen was driven by the displacement controller and moved downward at a constant rate of \SI{1}{mm/min}, while the lower platen remained fixed. As shown in Figure~\ref{fig11}, the testing machine automatically collected and recorded the compression load data throughout the tests, and a camera was used to capture the real-time deformation process of the specimens. Based on the original geometric dimensions of the specimens, the stress and strain values under uniform compression were calculated. The elastic modulus of the composite cylindrical truss lattice structures was determined from the slope of the stress--strain curve in the small-deformation elastic region.
